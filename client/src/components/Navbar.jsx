...
const Navbar = () => {
    ...
    const isAdmin = user && user.email.toLowerCase() === import.meta.env.VITE_ADMIN_EMAIL.toLowerCase();
  ...
  return (
    ...
    className={`flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 text-gray-600 border-b border-borderColor relative transition-all ${location.pathname === "/" && "bg-transparent"}`}>
        ...
        <div className={`max-sm:fixed max-sm:h-[calc(100vh-64px)] max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col items-start gap-4 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "bg-gray-50" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"} sm:hidden overflow-y-auto`}>
            {menuLinks.map((link, index)=> (
